...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
 ...
 <% 
    ...
    String url = "dingdanshengcheng_list2.jsp?1=1"; 
    String sql =  "select * from dingdanshengcheng where xiadanren='"+request.getSession().getAttribute("username")+"' ";
...
if(request.getParameter("zhuangtai")=="" ||request.getParameter("zhuangtai")==null ){}else{sql=sql+" and zhuangtai like '%"+request.getParameter("zhuangtai")+"%'";}
 ...
 <td align='center'><%=map.get("iszf")%>  <% if(map.get("iszf").equals("否")){%><a href="zhifu/index.jsp?id=<%=map.get("id")%>&biao=dingdanshengcheng"><font color='red'>支付</font></a> <%}%> </td>
    <td width='80' align='center'><a href="sh.jsp?id=<%=map.get("id")%>&yuan=<%=map.get("issh")%>&tablename=dingdanshengcheng" onClick="return confirm('您确定要执行此操作？')"><%=map.get("issh")%></td>
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="60" align="center"> <a href="dingdanshengcheng_detail.jsp?id=<%=map.get("id")%>">详细</a> </td>
  </tr>
  	<%
  }
   %>
